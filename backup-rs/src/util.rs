...
// When NO_COLOR env var is set, the e() function strips ANSI codes.
...
// When NO_COLOR env var is set, ANSI codes are stripped from output.
pub fn e(msg: &str) {
    let no_color = std::env::var("NO_COLOR").unwrap_or_default().len() > 0;
    if no_color {
